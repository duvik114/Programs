...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        if (!(route.getAction().equals("after")) && !(route.getAction().equals("before"))) {
        ...
        for (Class<?> clazz = pageClass; beforeMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                beforeMethod = pageClass.getDeclaredMethod("before", HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        for (Class<?> clazz = pageClass; afterMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                afterMethod = pageClass.getDeclaredMethod("after", HttpServletRequest.class, Map.class);
            } catch (NoSuchMethodException ignored) {
                // No operations.
            }
        }
        ...
        try {
            ...
            throw new ServletException("Unable to run action [pageClass=" + pageClass + ", method=" + beforeMethod + "].", e);
        }
        ...
        try {
            ...
            throw new ServletException("Unable to run action [pageClass=" + pageClass + ", method=" + afterMethod + "].", e);
        }
