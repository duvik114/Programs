...
public class Page {
    ...
    protected boolean checkCurUser(HttpSession httpSession, User user) {
        if (user == null || httpSession.getAttribute(USER_ID_SESSION_KEY) == null) {
            return false;
        } else {
            long id;
            try {
                id = (Long) httpSession.getAttribute(USER_ID_SESSION_KEY);
